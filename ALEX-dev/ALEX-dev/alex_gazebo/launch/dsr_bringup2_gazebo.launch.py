...
def generate_launch_description():
    ...
    included_launch_file_path = os.path.join(
        get_package_share_directory('alex_gazebo'),
        'launch',
    ...
    config_file = os.path.join( get_package_share_directory('alex_gazebo'), 'params', "ros_gz_bridge_config.yaml" )
    ...
    display_image_node = Node(
        package="image_view",
        executable="image_view",
        remappings=[("image", "/color/image_raw")]
    )

    return LaunchDescription(ARGUMENTS + nodes + [ros_gazebo_bridge, display_image_node])
