...
"""
Document: https://files.atlas-scientific.com/EZO-pH-complete-datasheet.pdf
"""
# ROS Import
import rclpy
...
from alex_interfaces.msg import PHMeterStatus, SerialMessage
from alex_interfaces.srv import PHMeterCommand
# Utilities Import
...
class AtlasPhProbe(Node):
    ...
    def set_publishers(self):
        ...
        self.pub_ph_reading = self.create_publisher(PHMeterStatus, "ph_reading", 1)

    def set_services(self):
        self.create_service(PHMeterCommand, "start_ph_reading", self.start_ph_reading )
    ...
    def calibration(self, level: CalibrationLevel, value: float):
        with self.serial_data.mutex:
            self.serial_data.queue.clear
        self.pub_to_serial.publish(SerialMessage(data=f"Cal,{level.value},{value}"))
        for _ in range(3):
            serial_data = self.serial_data.get(timeout=self.serial_timeout)
            match serial_data:
                case "*ER":
    ...
    def get_slope(self):
        with self.serial_data.mutex:
            self.serial_data.queue.clear
        self.pub_to_serial.publish(SerialMessage(data=f"SLOPE,?"))
        for _ in range(3):
            serial_data = self.serial_data.get(timeout=self.serial_timeout)
            self.logger.info(serial_data)
            if serial_data.startswith("*ER"):
                return False
            elif serial_data.startswith("?SLOPE"):
                return serial_data.removeprefix("?SLOPE,")
            serial_data = self.serial_data.get(timeout=self.serial_timeout) # to get `*OK`
        return False

    def start_ph_reading(self, request:PHMeterCommand.Request, response:PHMeterCommand.Response):
        if not self.device_connected:
            # TODO: Should handle when reconnected but device give data
            self.logger.info("pH meter not connected")
            response.status.status = PHMeterStatus.ERROR
            response.status.reason = "pH meter not connected"
            return response
        
        match request.command:
            case PHMeterCommand.Request.COMMAND_CUSTOM_MEASURE:
                ...
                self.logger.info("Started reading the pH data")
                response.success = True
                response.status.status = PHMeterStatus.WORKING
                response.status.reason = "STARTED"
            case PHMeterCommand.Request.COMMAND_CUSTOM_STOP:
                self.job_id = None
                self.logger.info("Stopped reading the pH data")
                response.success = True
                response.status.status = PHMeterStatus.COMPLETED
                response.status.reason = "STOPPED"
            case PHMeterCommand.Request.COMMAND_CALIBRATE:
                response.status.job_id = request.job_id
                level = request.optional
                try:
                    level = CalibrationLevel(level)
                except Exception as e:
                    response.status.status = PHMeterStatus.ERROR
                    response.status.reason = f"Error at Calibration level in value `optional`, {e}"
                    return response
                response.success = self.calibration(level, request.value)
                response.status.status = PHMeterStatus.COMPLETED
            case PHMeterCommand.Request.COMMAND_SLOPE:
                response.status.job_id = request.job_id
                slope = self.get_slope()
                if slope:
                    response.success = True
                    response.status.status = PHMeterStatus.COMPLETED
                    response.status.other_data = slope
                else: 
                    response.success = False
                    response.status.status = PHMeterStatus.ERROR
                    response.status.reason = "Did not get valid response from device"
            case _:
                response.status.job_id = request.job_id
                response.success = False
                response.status.status = PHMeterStatus.ERROR
    ...
    def handle_serial_out(self, data: SerialMessage):
        ...
        msg = PHMeterStatus()
        msg.status = PHMeterStatus.COMPLETED
        if not self.job_id:
            return
        if not self.device_connected:
            msg.status = PHMeterStatus.ERROR
            msg.reason = "Device Disconnected"
        msg.job_id = self.job_id
        msg.value.value = float(data.data)
        msg.value.unit  = "pH"
        self.pub_ph_reading.publish(msg)
