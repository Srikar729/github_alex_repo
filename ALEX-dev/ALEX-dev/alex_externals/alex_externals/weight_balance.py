...
class WeightBalance(Node):
    ...
    def from_serial_callback(self, msg: SerialMessage):
        ...
        data = msg.data.split()
        if len(data) < 2:
            self.logger.warning(f"Error in parsing the msg. {msg.data}")
            return
        value, unit, *other = data
        weight_msg.value.value = round(float(value), 4)
        weight_msg.value.unit = unit
        weight_msg.stable = '?' not in other
        weight_msg.tared = 'N' in other
